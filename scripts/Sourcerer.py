...
class Sourcerer:
	...
	def _getSource(self, source):
		...
		if isinstance(source, str):
			...
			source_names = [self._getParVal(s['Settings']['Name']) for s in sources]
	...
	def SwitchToSource(self, source):
		...
		if source_type == 'file':
			...
			#print('done_on', done_on, done_on=='timer')
			if done_on == 'timer':
				#source_comp.op('timerFile').par.start.pulse()
				source_comp.op('startTimerFile').run(delayFrames=1)

		else:
			done_on = self._getParVal(source['TOP']['Doneontop'])
			if done_on == 'timer':
				#source_comp.op('timerTOP').par.start.pulse()
		...
		if trans_type == 'glsl':
			...
			if glsl_trans in glsl_transitions.keys():
					...
					for p in transition_pars:
						# print(p)
						# pprint(source['GLSL Transition'][p])
			...
			with open(f, 'r') as json_file:
				...
				if imported_sources:
					...
					# append
						...
						#print('hello', len(sources), len(new_sources))
